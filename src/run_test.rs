...
use crate::kd3::kdtree::{
    kdtree_build, kdtree_delete, kdtree_iterator_delete, kdtree_iterator_get_next, kdtree_search,
    kdtree_search_space,
};
...
extern "C" fn initialise_points() -> () {
    unsafe {
        x = unsafe { malloc((core::mem::size_of::<f64>() as u64).wrapping_mul(11 as u64)) }
            as *mut f64;
        y = unsafe { malloc((core::mem::size_of::<f64>() as u64).wrapping_mul(11 as u64)) }
            as *mut f64;
        z = unsafe { malloc((core::mem::size_of::<f64>() as u64).wrapping_mul(11 as u64)) }
            as *mut f64;
...
extern "C" fn validate(iter: *mut KdtreeIterator, v: &[u64]) -> () {
    ...
    if !(iter as *mut () != 0 as *mut ()) as i32 as i64 != 0 {
        unsafe {
            __assert_rtn(
                c"validate".as_ptr() as *const i8,
                c"run_test.c".as_ptr() as *const i8,
                40,
    ...
    /// DBL_MAX
    (content = unsafe { malloc((core::mem::size_of::<u64>() as u64).wrapping_mul(v.len() as u64)) }
        as *mut u64);
    {
        i = 0 as u64;
        '__b1: loop {
            ...
            '__c1: loop {
                unsafe {
                    *content.add(i as usize) = kdtree_iterator_get_next(unsafe { &mut *iter })
                };
                break '__c1;
            }
            i = i.wrapping_add(1);
        ...
        != 0
    {
        unsafe {
            __assert_rtn(
                ...
                c"run_test.c".as_ptr() as *const i8,
                45,
                c"kdtree_iterator_get_next(iter) == KDTREE_END".as_ptr() as *const i8,
            )
        }
    } else {
        {
    ...
    };
    {
        ...
        '__b2: loop {
            ...
            '__c2: loop {
                if !(unsafe { *content.add(i as usize) } == v[i as usize]) as i32 as i64 != 0 {
                    unsafe {
                        __assert_rtn(
                            c"validate".as_ptr() as *const i8,
                            c"run_test.c".as_ptr() as *const i8,
...
pub(crate) extern "C" fn __main_inner() -> Result<(), i32> {
    unsafe {
        ...
        let mut iter: *mut KdtreeIterator = 0 as *mut () as *mut KdtreeIterator;
        initialise_points();
        ...
        let e2: [u64; 11] = [
            0 as u64, 1 as u64, 2 as u64, 3 as u64, 4 as u64, 5 as u64, 6 as u64, 7 as u64,
            8 as u64, 9 as u64, 10 as u64,
        ];
        ...
        let e3: [u64; 7] = [
            0 as u64, 1 as u64, 2 as u64, 3 as u64, 4 as u64, 5 as u64, 6 as u64,
        ];
        validate(iter, &e3[..7 as usize]);
        kdtree_search(tree, &mut iter, 0.5, 0.5, 1.0, 0.5);
        let e4: [u64; 7] = [
            0 as u64, 1 as u64, 2 as u64, 7 as u64, 8 as u64, 9 as u64, 10 as u64,
        ];
        ...
        let e5: [u64; 7] = [
            0 as u64, 1 as u64, 2 as u64, 5 as u64, 6 as u64, 9 as u64, 10 as u64,
        ];
